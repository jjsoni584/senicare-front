...
import { SignInReponseDto } from '../../apis/dto/response/auth';
import { useCookies } from 'react-cookie';
import { ACCESS_TOKEN, CS_ABSOLUTE_PATH, ROOT_PATH } from '../../constants';
...
function SnsContainer ({ type }: SnsContainerProps) {
    ...
    const onSnsButtonClicHandler = (sns: 'kakao' | 'naver') => {
    ...
    return (
        <div className="sns-container">
            ...
            <div className="sns-button-container">
                <div className={`sns-button ${type === '회원가입' ? 'md ' : ''}kakao`}onClick={()=> onSnsButtonClicHandler('kakao')}></div>
                <div className={`sns-button ${type === '회원가입' ? 'md ' : ''}naver`}onClick={()=> onSnsButtonClicHandler('naver')}></div>
            </div>
        </div>
    );
...
function SignUp({onPathChange}: AuthComponentProps) {
    ...
    const [isMatchedPassword, setIsMathchedPassword] = useState<boolean>(false);
    const [isCheckPassword, setCheckPassword] = useState<boolean>(false);
    const [isSend, setIsSend] = useState<boolean>(false);
    const [ischeckAuthNumber, setCheckAuthNumber] = useState<boolean>(false);

    // variable: SNS 회원가입 여부 //
    const isSnsSignUp = snsId !== null && joinPath !== null;

    // variable: 회원가입 가능 여부 //
    const isComplete = name && id && isCheckedId && passwordCheck && password && isCheckPassword
        && telNumber && isSend && authNumber && ischeckAuthNumber && isMatchedPassword;
    ...
    setCheckAuthNumber(isSuccessed);
    }
    ...
    const onPasswordChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const message = (isMatch || !value) ? '' : '영문, 숫자를 혼용하여 8 ~13자 입력해주세요';
        setPasswordMessage(message);
        setPasswordMessageError(!isMatch);
        setIsMathchedPassword(isMatch)
    ...
    const onAuthNumberChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        setCheckAuthNumber(!ischeckAuthNumber);
    }
...
function SignIn({onPathChange}: AuthComponentProps) {
    ...
    const signInResponse = (responseBody: SignInReponseDto | ResponseDto | null) => {
        const message = 
            !responseBody ? '서버에 문제가 있습니다.' :
        ...
        const {accessToken, expiration} = responseBody as SignInReponseDto;
