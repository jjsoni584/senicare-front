...
import { ACCESS_TOKEN, HR_ABSOLUTE_PATH } from '../../../constants';
import { ResponseDto } from '../../../apis/dto/response';
import { GetChargedCustomerResponseDto, GetNurseResponseDto } from '../../../apis/dto/response/nurse';
import Pagination from '../../../components/pagination';
import { usePagination } from '../../../hooks';
import { getChargedNurseRequest, getNurseRequest, patchCustomerRequest, patchNurseRequest } from '../../../apis';
import { useHrDeailUpdateStore, useSignInUserStore } from '../../../stores';
import { calculateAge } from '../../../utils';
import { ChargedCustomer } from '../../../types';
import { PatchNurseRequesDto } from '../../../apis/dto/request/nurse';
...
export default function HRDetail() {
    ...
    // state: 수정 화면 상태 //
    const {update, setUpdate} = useHrDeailUpdateStore();
    ...
    // variable: 본인 여부 //
    const isSiginInUser = userId === signInUser?.userId;

    // function: navigatoer 함수 //
    const navigatoer = useNavigate();
    ...
    const getChargedNurseResponse = (responseBody: GetChargedCustomerResponseDto | ResponseDto | null) =>{
        const message = 
            !responseBody ? '서버에 문제가 있습니다.' :
            responseBody.code === 'AF' ? '잘못된 접근입니다.' :
            responseBody.code === 'DBE' ? '서버에 문제가 있습니다.' : '';
    ...
    const onUpdateButtonClickHandler = () => {
        ...
        if(!isSiginInUser) return;
        const accessToken = cookies[ACCESS_TOKEN];
        if (!accessToken) return;

        const requestBody: PatchNurseRequesDto = {
            name: updateName
        };
    ...
    // effect: 요양사 아이디 변경될 시 실행할 요양사 정보 불러오기 //
    useEffect(()=>{
        if(!userId) return;
        const accessToken = cookies[ACCESS_TOKEN];
        if (!accessToken) return;
        ...
        getNurseRequest(userId, accessToken).then(getNurseResponse);
        getChargedNurseRequest(userId, accessToken).then(getChargedNurseResponse);
    }, [userId])
