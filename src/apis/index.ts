...
import { SignInReponseDto } from "./dto/response/auth";
...
import { PatchCustomerRequestDto, PostCareRecordRequestDto } from "./dto/request/customer";
import { useRouteId } from "react-router/dist/lib/hooks";
import { PatchNurseRequesDto } from "./dto/request/nurse";
...
const GET_CHARGED_API_URL = (userId: string) => `${NURSE_MODUEL_URL}/${userId}/customers`
...
export const signInRequest = async (requestBody: SignInRequestDto) => {
    const responseBody = await axios.post(SIGN_IN_API_URL, requestBody)
        .then(responseDataHandler<SignInReponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
};
...
export const patchNurseRequest = async(requestBody: PatchNurseRequesDto, accessToken: string) => {
...
// function: get charged nurse 요청 함수 //
export const getChargedNurseRequest = async (userId: string, accessToken: string) => {
    const responseBody = await axios.get(GET_CHARGED_API_URL(userId), bearerAuthorization(accessToken))
...
export const patchCustomerRequest = async (requestBody: PatchCustomerRequestDto, customerNumber: string, accessToken: string) => {
...
export const postCareRecordRequest = async (requestBody: PostCareRecordRequestDto, customerNumber : number | string, accessToken: string) => {
    const responseBody = axios.post(POST_CARE_RECORD_API_URL(customerNumber), requestBody, bearerAuthorization(accessToken))
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
};
